 ...
 */
import { detectCoOccurrence } from "./coOccurrence";
import { detectWinRateFlags } from "./winRate";
...
describe("co-occurrence", () => {
  it("flags pair when N_together ≥ 15 and avg(min(place)) ≤ 2.2", () => {
    ...
    const flags = detectCoOccurrence(matchesPerUser);
...
describe("win-rate", () => {
  it("flags when ≥20 matches and ≥80% 1st over rolling 50", () => {
    ...
    const flags = detectWinRateFlags(userPlaces);
