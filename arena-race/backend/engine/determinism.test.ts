...
import type { MatchState, PlayerAction, BoardConfig } from "./types";

const BOARD_CONFIG: BoardConfig = { trapTiles: [12, 24, 36] };
...
function runMatch(
  ...
  actionLog: [PlayerAction, PlayerAction, PlayerAction, PlayerAction][]
): { finalState: MatchState; placement: [number, number, number, number] } {
...
describe("determinism", () => {
  it("1,000 identical (state + actions) runs produce identical output", () => {
    const tokenPositions = START as MatchState["tokenPositions"];
    const state = createInitialState(BOARD_CONFIG, tokenPositions);
    ...
    const first = resolveTurn(state, actions);
    for (let i = 0; i < 999; i++) {
  ...
  it("replay full match from stored action log reproduces final score and placement", () => {
    const tokenPositions = START as MatchState["tokenPositions"];
    const initialState = createInitialState(BOARD_CONFIG, tokenPositions);
    const actionLog: [PlayerAction, PlayerAction, PlayerAction, PlayerAction][] = [];
    for (let t = 0; t < 10; t++) {
      actionLog.push([
